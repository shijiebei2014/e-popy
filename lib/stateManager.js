...
const debug = require('debug')('popy:StateManager')
...
proto.handle = function(data, callback) {
  ...
  if (typeof cb === 'function') {
    if (!!~[STATE.RETR].indexOf(client.state)) {
      if (!client.mailparser) {
        const sData = client.data.toString()
        cb(sData.substring('+OK\r\n'.length, sData.length - '\r\n.\r\n'.length))
        delete client.data
        return client.cmds.runBatch()
      }
    ...
    } else if (!!~[STATE.LIST, STATE.TOP].indexOf(client.state)) {
      cb(client.data.toString())
      delete client.data
      return client.cmds.runBatch()
    }
    client.state = STATE.NOOP
